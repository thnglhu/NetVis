...
class Graph(ig.Graph):
    ...
    def _init_subclass(self):
        def shorten(element, attributes, *key_default):
            for pair in key_default: element[pair[0]] = attributes.get(pair[0], pair[1]) or pair[1]
    ...
    def display(self, canvas):
        for e in self.edges: e.display(canvas)
        for v in self.vertices: v.display(canvas)
        canvas.fix_order()
    ...
    def fit_canvas(self, canvas):
        vertices = [[], []]
        try: vertices[0] = self.vs['x']
        except: vertices[0] = [0] * len(self.vs)
        try: vertices[1] = self.vs['y']
        except: vertices[1] = [0] * len(self.vs)
